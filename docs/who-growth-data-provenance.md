...
`src/lib/whoGrowthData.ts`의 0–24개월 체중-연령(WFA) 및 신장/길이-연령(LHFA) LMS 값은 아래 WHO 공식 Excel 원본의 지정 워크시트에서 추출했다. 저장소에는 재현 가능한 오프라인 검증을 위해 공식 바이너리 4개를 [`tests/fixtures/who-growth-official`](../tests/fixtures/who-growth-official)에 바이트 그대로 보관하고, 원본 URL·SHA-256·워크시트 이름·정규화한 100개 행을 [`tests/fixtures/who-growth-standards.manifest.json`](../tests/fixtures/who-growth-standards.manifest.json)에 고정한다. 바이너리의 출처와 저작자는 WHO이며, 테스트 fixture는 아래 공식 다운로드와 SHA-256으로 귀속·식별한다.
...
2026-07-13 검토에서는 위 URL에서 새로 받은 각 파일의 해시가 같은 날 제공된 로컬 공식 사본, 저장소 fixture 및 매니페스트 값과 모두 일치했다.

## 정규화 규칙

- `xl/workbook.xml`과 관계 파일에서 매니페스트에 적힌 워크시트 이름을 정확히 찾아 사용한다.
...
[`scripts/verify-who-growth-workbooks.mjs`](../scripts/verify-who-growth-workbooks.mjs)는 Node.js 내장 `crypto`와 `zlib`만 사용해 ZIP/XLSX를 직접 읽는다. 파일별 SHA-256을 먼저 확인한 뒤 workbook 관계, 지정 워크시트, shared strings, 필수 헤더를 해석하고 0–24개월 25행을 순서대로 추출한다. 추출한 네 시리즈를 매니페스트와 앱 배열 양쪽에 허용 오차 없이 비교한다. 별도 런타임 또는 개발 의존성은 추가하지 않았다.

[`tests/whoGrowthOfficialWorkbook.test.ts`](../tests/whoGrowthOfficialWorkbook.test.ts)는 실제 fixture와 재해시한 변형 XLSX를 사용해 다음 실패 조건도 검증한다.

- 잘못된 바이트·SHA-256 및 성별/지표 파일 오매핑
- 잘못된 워크시트와 `Month`/`L`/`M`/`S` 헤더
- 월령 누락·중복·재정렬·초과 행
- workbook, 매니페스트 또는 앱 배열의 단일 값 변동

기존 `whoGrowthData.ts`의 네 배열은 공식 원본 100행과 이미 정확히 일치했으므로 제품 숫자는 수정하지 않았다. [`tests/whoGrowthOfficialData.test.ts`](../tests/whoGrowthOfficialData.test.ts)는 앱 배열과 매니페스트의 추가 경계 검증을 유지한다.
...
macOS/Linux에서는 같은 파일에 `shasum -a 256 <파일>`을 사용할 수 있다. 운영체제와 관계없이 저장소 루트에서 다음 한 명령으로 공식 XLSX 바이트→100행→매니페스트·앱 배열 결속과 기존 경계 검증을 재현한다.

```text
npx vitest run tests/whoGrowthOfficialWorkbook.test.ts tests/whoGrowthOfficialData.test.ts
```
