...
describe('packaged sync E2E selector contract', () => {
  ...
  it('uses independent full-payload edit and delete expectations on both devices', () => {
    const runner = source('scripts/sync-e2e.mjs')
    expect(runner).toContain('const expectedSource = buildExpectedEditedEvent(prior,')
    expect(runner).toContain('stableJson(localSource) === stableJson(expectedSource)')
    expect(runner).toContain('const expectedSource = buildExpectedDeletedEvent(prior,')
    expect(runner).toContain('const editedOperation = await editEventTime(b, firstA)')
    expect(runner).toContain('const expectedEdited = editedOperation.canonical')
    expect(runner).toContain('await waitForSemanticEvent(a, expectedEdited)')
    expect(runner).toContain('const deletedOperation = await deleteEvent(a, firstB)')
    expect(runner).toContain('const expectedDeleted = deletedOperation.canonical')
