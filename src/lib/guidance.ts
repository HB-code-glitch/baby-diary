 ...
 * Returns null outside the source age window. The structured bands are retained
 * for compatibility, but must not be extrapolated into later ages.
 *
 * Band boundaries match the startDay of each marker:
 *   formula_0_1mo: 0–29 days
 *   formula_1_3mo: 30–89 days
 *   formula_3_6mo: 90–179 days
 */
export function getFeedingBand(ageDays: number): FeedingBand | null {
  ...
  if (ageDays < 180) return FEEDING_BANDS[3] // formula_3_6mo
  return null
}
...
/** Structured red flags shared by evaluation and presentation. */
export const FEVER_RED_FLAGS: readonly FeverRedFlag[] = [
  {
    id: 'pale_blue_or_mottled',
    ko: '피부나 입술이 창백하거나 파랗고 얼룩덜룩해 보여요.',
    ja: '皮膚や唇が青白い、またはまだらに見えます。',
  },
  {
    id: 'breathing_difficulty',
    ko: '숨쉬기 힘들어하거나 끙끙거리고, 가슴이 심하게 들어가요.',
    ja: '呼吸が苦しそう、うなり声がある、胸が強くへこみます。',
  },
  {
    id: 'hard_to_wake',
    ko: '깨우기 어렵거나 평소와 달리 축 늘어져 반응이 적어요.',
    ja: '起こしにくい、ぐったりして普段より反応が乏しい状態です。',
  },
  {
    id: 'non_blanching_rash',
    ko: '투명한 컵으로 눌러도 옅어지지 않는 발진이 있어요.',
    ja: '透明なコップで押しても消えない発疹があります。',
  },
  {
    id: 'seizure',
    ko: '경련을 해요.',
  ...
  {
    id: 'stiff_neck_or_bulging_fontanelle',
    ko: '목이 뻣뻣하거나 숨구멍이 불룩해 보여요.',
    ja: '首が硬い、または大泉門がふくらんで見えます。',
  },
  {
    id: 'severe_dehydration',
    ko: '소변이 뚜렷하게 줄고 입이 마르거나 눈물이 나지 않아요.',
    ja: '尿が明らかに減り、口が乾く、または涙が出ません。',
  },
  {
    id: 'bilious_or_projectile_vomiting',
    ko: '초록색 구토를 하거나 세게 뿜는 구토가 반복돼요.',
    ja: '緑色の嘔吐、または勢いよく噴き出す嘔吐を繰り返します。',
  },
  {
    id: 'poor_feeding',
    newbornOnly: true,
    ko: '신생아가 잘 먹지 못하거나 수유를 계속 거부해요.',
    ja: '新生児がうまく飲めない、または授乳を繰り返し拒みます。',
  },
] as const

export const FEVER_CARE: { steps: FeverCareStep[]; sourceLabel: string } = {
  sourceLabel: 'NICE NG143 · NHS',
  steps: [
    {
      ko: '너무 덮거나 지나치게 벗기지 말고 편안한 옷차림을 유지해요.',
      ja: '厚着させすぎたり脱がせすぎたりせず、楽な服装を保ちます。',
    },
    {
    ...
    {
      ko: '호흡·반응·피부색·소변과 발진을 살피고 상태가 나빠지면 바로 도움을 받아요.',
      ja: '呼吸・反応・肌の色・尿・発疹を見守り、悪化したらすぐに助けを求めます。',
    },
  ],
}
...
interface FeverAgeContext {
  ageDays: number
  completedMonths: number
}

function getFeverAgeContext(
  birthdate: string | null,
  ...
  return {
    ageDays,
    completedMonths: differenceInMonths(measured, birth),
  }
}
 ...
 *
 * Day-based and calendar-month rules intentionally remain separate:
 * under 90 days uses completed calendar days, while 3–6 months uses completed
 * calendar months. Invalid/future birth dates are treated as unknown age.
 */
export function evaluateFever({
  ...
  const hasUrgentRedFlag = symptomIds.some(id => {
    ...
    return flag != null && (!flag.newbornOnly || isNewborn)
  })
  if (hasUrgentRedFlag) return 'emergency'

  if (isNewborn && celsius < 35.5) return 'emergency'
  ...
  // From six completed calendar months, temperature alone is not a danger tier.
  return 'caution'
}
